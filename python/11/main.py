...
class Canvas():
    ...
    def paintPoint(self, point, color):
        print("painting point: " + str(point) + "in color: " + str(color))
...
class PaintRobot(OutputDevice, InputDevice):
    ...
    def receiveinput(self, param: int):
        ...
        if len(self.inputs) == 2:
            self.canvas.paintPoint(self.location.copy(), Color(self.inputs[0]))
    ...
    def notify(self):
        self.computer.receiveinput(self.canvas.getcolor(self.location.copy()).value)
...
robot = PaintRobot()
robot.computer = OpcodeSolver(program)
robot.computer.inputdevice = robot
robot.computer.outputdevice = robot

robot.canvas.points[Point(0,0)] = Color.WHITE
while robot.computer.state != State.FINISHED:
    robot.computer.readprogram()

print(len(robot.canvas.points))
print(Point(-114, 17) in robot.canvas.points)
#plt.plot([point.x for point in robot.canvas.points if robot.canvas.points[point] == Color.BLACK], [point.y for point in robot.canvas.points if robot.canvas.points[point] == Color.BLACK], '.', color="black")
plt.plot([point.x for point in robot.canvas.points if robot.canvas.points[point] == Color.WHITE], [point.y for point in robot.canvas.points if robot.canvas.points[point] == Color.WHITE], '.', color = "r")
